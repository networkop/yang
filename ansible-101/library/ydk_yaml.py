...
class YdkModel:
    ...
    def verify(self, device):
        if self.model == 'interfaces-state':
            ...
            raise ValueError('Untested or not implemented operational model {}'.format(self.model))
    ...
    def action(self, crud_action, device):
        if crud_action not in ['create','read']:
            raise ValueError('Untested or not implemented CRUD action {}'.format(crud_action))
